...
Node *primary(){
    ...
    if(consume("(")){
        ...
        char buf[20];
        snprintf(buf, sizeof(buf), ".L.str.%d", str_id++);
        char *s = malloc(strlen(buf) + 1);
        strcpy(s, buf);
        g->name = s;
        g->len = strlen(g->name);
        g->ty = array_of(char_type(), tok->len + 1);
        g->init_data = tok->str;
        g->init_len = tok->len;
        g->next = strings;
        strings = g;
...
Node *stmt(){
    ...
    if(peek()->kind == TK_INT || peek()->kind == TK_CHAR){
        Type *ty = basetype();
...
void program(){
    ...
    while(!at_eof()){
        ...
        if(peek()->kind == TK_RESERVED && peek()->str[0] == '('){
            funcs[i++] = function(ty, name);
        }else{
            gvar(ty, name);
        }
    }
...
Type *basetype(){
    ...
    if(consume_kw(TK_CHAR)) ty = char_type();
    else {
        expect_kw(TK_INT);
        ty = int_type();
    }
...
void add_type(Node *node, bool decay){
    if(!node) return;
    if(node->ty && node->kind != ND_LVAR && node->kind != ND_GVAR) return;
